...
#'      \item \code{list.style}: \code{'bullet'}, \code{'ordered'} or \code{'roman'} passed to \code{\link{pandoc.list}}. Please not that this has no effect on \code{rcov} methods.
...
#'      \item \code{table.split.table}: numeric passed to \code{\link{pandoc.table}} and also affects \code{rcov} methods. This option tells \code{rcov} where to split too wide tables. The default value (\code{80}) suggests the conventional number of characters used in a line, feel free to change (e.g. to \code{Inf} to disable this feature) if you are not using a VT100 terminal any more :)
#'      \item \code{table.split.cells}: numeric or numeric vector (default: \code{30}) passed to \code{\link{pandoc.table}} and also affects \code{rcov} methods. This option tells \code{rcov} where to split too wide cells with line breaks. Numeric vector specifies values for cells separately. Set \code{Inf} to disable.
...
#'      \item \code{evals.messages}: boolean (default: \code{TRUE}) passed to \code{evals}' \code{rcov} method specifying if messages should be rendered
...
#'      \item \code{knitr.auto.asis}: boolean (default: \code{TRUE}) if the results of \code{rcov} should be considered as \code{'asis'} in \code{knitr}. Equals to specifying \code{results='asis'} in the R chunk, so thus there is no need to do so if set tot \code{TRUE}.
